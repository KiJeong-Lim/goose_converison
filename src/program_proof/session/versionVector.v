...
Section heap.
  ...
  Lemma wp_lexicographicCompare (x: Slice.t) (xs: list u64) (y: Slice.t) (ys: list u64) (dx: dfrac) (dy: dfrac) :
    {{{
      ...
      CoqSessionServer.lexicographicCompare x y 
    {{{
        r, RET #r;
        own_slice_small x uint64T dx xs ∗
        own_slice_small y uint64T dy ys ∗
  ...
  Proof.
    ...
    - iIntros (?). iModIntro. iIntros "H1 H2". iNamed "H1". iDestruct "H1" as "(H1 & H3 & H4 & H5 & %H6 & %H7 & %H8 & %H9)". wp_load. wp_load. wp_if_destruct.
      ...
      + iModIntro. iApply "H2". iExists b. iExists i. iFrame. iPureIntro. repeat (split; (word || auto)). intros. left. split; word || auto.
  ...
  Proof.
    ...
    - iIntros (?). iModIntro. iIntros "H1 H2". iNamed "H1". iDestruct "H1" as "(H3 & H4 & H5 & %H6 & %H7)".
      ...
      + wp_pures. wp_load. assert ((uint.nat i < length xs)%nat) by word.
        ...
        * iIntros "Hx". wp_pures. wp_load. assert ((uint.nat i < length ys)%nat) by word.
          ...
          { wp_pures. wp_store. iModIntro. iApply "H2". iExists false. iExists i. iFrame. iPureIntro. split; auto. split; auto. split.
            - intros. rewrite H in H3. rewrite H0 in H4. injection H3 as ?. injection H4 as ?. unfold not in H1. assert (x2 = y0) by word. rewrite H13 in H3. rewrite <- H4 in H3. rewrite H3 in H1. assert (#x1 = #x1) by word. apply H1 in H14. inversion H14.
      ...
      + iModIntro. iApply "H2". iExists b. iExists i. iFrame. iPureIntro. split; auto. split; auto. split; auto. split; auto. split; auto. split; auto. right. split; try word. auto.
          ...
          - rewrite e. simpl. destruct (decide (uint.nat i = 0%nat)).
            ...
            + eapply IHxs; auto.
              ...
              * intros. destruct H11; auto.
                ...
                { right. rewrite length_cons in H1. destruct H1. split; try word. auto. }
          - assert ((uint.Z a =? uint.Z a0) = false) by word.
        ...
        * assert (length xs_prev <= length xs). { rewrite H6. rewrite length_app. word. }
          ...
          { wp_load. wp_apply (wp_SliceGet with "[Hxs]"); iFrame; auto. iIntros "Hxs". wp_load.
            ...
            - wp_store. wp_load. wp_store. iModIntro. iApply "H_ret".
              ...
              { rewrite fold_left_app. destruct (fold_left loop_step (zip xs_prev ys_prev) loop_init) as [ind ?]. unfold loop_step. simpl. apply w64_val_neq in Heqb1 as [? ?]. apply Z.eqb_neq in H15. rewrite H15. simpl in H11. rewrite <- H11. simpl. assert (uint.Z x0 >=? uint.Z y0 = false) by word. rewrite H17. auto. }
              { rewrite fold_left_app. destruct (fold_left loop_step (zip xs_prev ys_prev) loop_init) as [ind ?]. unfold loop_step. simpl. apply w64_val_neq in Heqb1 as [? ?]. apply Z.eqb_neq in H15. rewrite H15. simpl in H11. rewrite <- H11. simpl. assert (uint.Z x0 >=? uint.Z y0 = false) by word. rewrite H17. auto. }
            - wp_load. wp_store. iModIntro. iApply "H_ret". iExists b1. iExists true. iExists (xs_prev ++ [x0]). iExists (drop 1 xs_next). iExists (ys_prev ++ [y0]). iExists (drop 1 ys_next). iFrame.
              ...
              rewrite H14. split.
              { rewrite fold_left_app. destruct (fold_left loop_step (zip xs_prev ys_prev) loop_init) as [ind ?]. unfold loop_step. simpl. apply w64_val_neq in Heqb1 as [? ?]. apply Z.eqb_neq in H15. rewrite H15. simpl in H11. rewrite <- H11. simpl. assert (uint.Z x0 >=? uint.Z y0 = true) by word. rewrite H17. simpl. rewrite H10. simpl. auto. }
              { rewrite fold_left_app. destruct (fold_left loop_step (zip xs_prev ys_prev) loop_init) as [ind ?]. unfold loop_step. simpl. apply w64_val_neq in Heqb1 as [? ?]. apply Z.eqb_neq in H15. rewrite H15. simpl in H11. rewrite <- H11. simpl. assert (uint.Z x0 >=? uint.Z y0 = true) by word. rewrite H17. auto. }
          }
        * assert (length xs_prev <= length xs). { rewrite H6. rewrite length_app. word. }
          assert (uint.nat (length xs_prev) < length xs)%nat by word.
          assert (uint.nat (length xs_prev) < length ys)%nat by word.
