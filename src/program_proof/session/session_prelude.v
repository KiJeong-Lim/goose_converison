...
Module SessionPrelude.
  ...
  Section MORE_LIST_LEMMAS.
    ...
    Lemma forallb_exists (p : A -> bool) (xs : list A)
      : forallb p xs = false <-> (exists x, In x xs /\ p x = false).
    ...
    Proof with try done.
      ...
      - exists 0%nat...
      - pose proof (IH x0 IN) as (n & EQ & LE). exists (S n). split... word.
    Qed.
  ...
  Section SORTED.
    ...
    Proof with eauto.
      ...
      - eapply IH.
        replace xs with ([] ++ xs)... replace (x :: xs) with ([] ++ [x] ++ xs) in SORTED...
        eapply isSorted_middle...
    Qed.
    ...
    Lemma sortedInsert_spec (l : list A) (i : A) (l_wf : Forall well_formed l) (i_wf : well_formed i)
      ...
      : ∃ prefix, ∃ suffix, l = prefix ++ suffix /\
        ...
        (∀ n : nat, ∀ x, suffix !! n = Some x -> if NOT_IN then ltb i x = true else ltb i x = true \/ eqb i x = true) /\
    ...
    Proof with eauto.
      ...
      - assert (SUBLIST : forall x : A, In x l -> In x l') by now subst l'.
        rewrite forallb_forall in H_In. clear H_l'; revert i SORTED i_wf l' SUBLIST H_In.
      ...
      - subst l'. rewrite forallb_exists in H_In. destruct H_In as (x2 & x2_in & EQ). rewrite negb_false_iff in EQ.
        rewrite eqb_eq in EQ; cycle 1... { eapply List.Forall_forall... } simpl.
        revert SORTED i i_wf x2 x2_in EQ; induction l_wf as [ | x1 xs x1_wf xs_wf IH]; simpl; intros SORTED' x0 x0_wf x2 x2_in EQ.
        { tauto. }
        assert (SORTED : isSorted xs).
        { intros i j i_lt_j x x' H_x H_x'. eapply SORTED' with (i := S i) (j := S j); try (word || done). }
        ...
        { assert (x2_wf : well_formed x2).
          ...
          { pose proof (IH SORTED x0 x0_wf x2 x2_in EQ) as (prefix & suffix & -> & IH' & H_prefix & H_suffix & WF).
